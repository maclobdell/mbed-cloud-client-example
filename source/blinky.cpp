...
Blinky::Blinky()
    : _pattern(NULL), _curr_pattern(NULL), _client(NULL),
      _button_resource(NULL), _state(STATE_IDLE), _restart(false)
{
    _button_count = 0;
}
...
{
    if (_tasklet < 0) {
        _tasklet = eventOS_event_handler_create(blinky_event_handler_wrapper,
                                                BLINKY_TASKLET_LOOP_INIT_EVENT);
        assert(_tasklet >= 0);
    }
}
...
bool Blinky::run_step()
{
    ...
    // tr_debug("patt: %s, curr: %s, delay: %d", _pattern, _curr_pattern,
    // delay);
    ...
    if (request_timed_event(BLINKY_TASKLET_PATTERN_TIMER,
                            ARM_LIB_MED_PRIORITY_EVENT, delay) == false) {
        _state = STATE_IDLE;
        assert(false);
        return false;
    }
...
{
    request_timed_event(BLINKY_TASKLET_LOOP_TIMER, ARM_LIB_LOW_PRIORITY_EVENT,
                        BUTTON_POLL_INTERVAL_MS);
}

// helper for requesting a event by given type after given delay (ms)
bool Blinky::request_timed_event(uint8_t event_type,
...
void Blinky::handle_buttons()
{
    ...
    // this might be stopped now, but the loop should then be restarted after
    // re-registration
