...
void create_m2mobject_test_set(M2MObjectList &object_list)
{
    ...
    for (int object_id = object_id_range_start; object_id < object_id_range_end;
         object_id++) {
        ...
        for (int object_instance_id = 0;
             object_instance_id < object_instance_count; object_instance_id++) {

            M2MObjectInstance *obj_inst =
                obj->create_object_instance(object_instance_id);

            assert(obj_inst != NULL);
            total_object_instance_count++;
            for (int resource_id = 0; resource_id < resource_count;
                 resource_id++) {
                String res_name;
                String res_type;
                ...
                M2MResource *resource = obj_inst->create_dynamic_resource(
                    res_name, res_type, M2MResourceInstance::INTEGER, true);
...
void print_m2mobject_stats()
{
    ...
    M2MSecurity *security =
        M2MInterfaceFactory::create_security(M2MSecurity::M2MServer);
    mbed_stats_heap_get(&stats);
    printf("M2MSecurity heap size: %" PRIu32 "\n",
           stats.current_size - initial);

    // If the MbedCloudClient is already instantiated, this corrupts heap as it
    // frees the singleton of M2MSecurity, which is still used by it. Use with
    // care.
    ...
    M2MResource *res = obj_inst->create_dynamic_resource(
        "1", "1", M2MResourceInstance::STRING, false);
    assert(res);
    mbed_stats_heap_get(&stats);
...
void print_stack_statistics()
{
    ...
    if (stats) {
        ...
        for (int i = 0; i < cnt; i++) {
            printf("Thread: 0x%" PRIx32 ", Stack size: %" PRIu32
                   ", Max stack: %" PRIu32 "\r\n",
                   stats[i].thread_id, stats[i].reserved_size,
                   stats[i].max_size);
        }
